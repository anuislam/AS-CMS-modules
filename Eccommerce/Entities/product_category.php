...
class product_category extends TarmModel
{
    ...
    public function tarm_html_out_put($errors, $route, $value = "", $methos = 'POST'){
        ...
        echo Form::open(['url' => route($route, $routeid), 'method' => $methos]); 
            text_field([
                ...
                'atts' =>  ['placeholder' => 'Category Name', 'class' => 'form-control']
            ], $errors);
            ...
            text_field([
                ...
                'atts' =>  ['placeholder' => 'Category Slug', 'class' => 'form-control']
            ], $errors);
            ...
            textarea_field([
                ...
                'atts' =>  ['placeholder' => 'Category Description','class' => 'form-control']
            ], $errors);

                
            if (empty($value) === false) {
                do_action('product_category_meta', [
                    'tarm_id' => $value['id'],
                    'errors' => $errors,
                ]);
            }else{
                do_action('product_category_meta', $errors);
            }
    ...
    public function tarm_data_process($request, $tarm_type){
        ...
        do_action('product_cat_meta_validation', $request->all());
    ...
    public function tarm_data_save($data, $tarm_type){
        ...
        $id = DB::table('tarms')->insertGetId([
            ...
            'tarm-type' => sanitize_text('product-cat'),
            'created_at' => new \DateTime(),
            'updated_at' => new \DateTime(),
        ]);
        if ($id) {
            do_action('product_cat_meta_save', $data);
    ...
    public function all_tarms_out_put(){
        ...
        ?>
          <table class="table table-bordered table-hover" id="tarm_opject_table" width="100%" cellspacing="0" tarms-url='<?php echo route('tarms-all', 'product-cat'); ?>' tarms-data='<?php echo json_encode([
            ['data' => 'tarm-name'],
            ['data' => 'tarm-slug'],
            [
                'data'       => 'action',
    ...
    public function tarm_data_for_datatable($tarm_type){
        return DataTables::of(DB::table('tarms')->select('id', 'tarm-slug', 'tarm-name', 'tarm-type')->where('tarm-type', 'product-cat'))
        ->addColumn('action', function ($tarm) {
            return '<a href="'.route('edit-tarm', [$tarm->id, 'product-cat']).'/" class="btn bg-purple btn-flat">Edith</a> <a
    ...
    public function tarm_edit_data_update($data, $tarm_id){
        ...
        do_action('product_cat_meta_update', $tarm_id);
